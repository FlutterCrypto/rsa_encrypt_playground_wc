                    ...
                    Expanded(
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          await RsaOaepPrivateKey.generateKey(2048, BigInt.parse('65537'), Hash.sha1);
                          // private key
                          final privateKey = await keyPair.privateKey.exportPkcs8Key();
                          final privateKeyBase64 = base64Encoding(privateKey);
                          var chunks = chunk(privateKeyBase64, 64);
                          final privateKeyPem = '$BEGIN_PRIVATE_KEY\n${chunks.join('\n')}\n$END_PRIVATE_KEY';
                          //public key
                          final publicKey = await keyPair.publicKey.exportSpkiKey();
                          final publicKeyBase64 = base64Encoding(publicKey);
                          chunks = chunk(publicKeyBase64, 64);
                          final publicKeyPem = '$BEGIN_PUBLIC_KEY\n${chunks.join('\n')}\n$END_PUBLIC_KEY';
